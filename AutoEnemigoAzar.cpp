...
AutoEnemigoAzar::AutoEnemigoAzar(int vel, int x0, int y0):ObjetoGrafico(vel){
...
bool AutoEnemigoAzar::update(int direccion){
	//recibe parámetro "direccion" que es el valor de x del auto del jugador para intentar chocarlo
	if (y<39){
		if(tempo+paso<clock()){
			borrar();
			if (y < 34){
